...
<body>
    ...
    @if($userData->id === $userDataRoom->id)
    ...
    <div class="max-w-7xl  md:mx-auto lg:mx-auto xl:mx-auto mt-14 bg-white shadow-xl rounded-lg text-gray-900">
        ...
        <div class="mx-auto w-32 h-32 relative -mt-16 border-4 border-white rounded-full overflow-hidden">
            <img class=" h-32" src='{{ $userDataRoom->owner_image }}' alt='image'>
        </div>
        <div class="text-center mt-2">
            <h2 class="font-semibold capitalize"> {{ $userDataRoom->owner_name }}</h2>
            <p class="text-gray-500"> {{ $userDataRoom->owner_number }}</p>
        </div>
        <ul class="py-4 mt-2 text-gray-700 flex items-center justify-around">
            <li class="flex flex-col items-center justify-around">
                <svg class="w-4 fill-current text-blue-900" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
