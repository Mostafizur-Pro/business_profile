...
<div class="container mx-auto px-4 py-6">
    <div class="bg-white shadow-md rounded px-8 py-6">
        ...
        <form action="{{ route('updateUserProfile', $editUserInfo->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-between">
                <div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="gender" name="gender" value="{{$editUserInfo->gender}}" class="w-full border rounded py-2 px-3 text-gray-800">
                    </div>
                    <div class="mb-4">
                        <label for="owner_number" class="block text-gray-700 font-bold mb-2">Number:</label>
                        <input type="text" id="owner_number" name="owner_number" value="{{$editUserInfo->owner_number}}" class="w-full border rounded py-2 px-3 text-gray-800" maxlength="11">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="owner_email" name="owner_email" value="{{$editUserInfo->owner_email}}" class="w-full border rounded py-2 px-3 text-gray-800" disabled>
                    </div>
                        ...
                        <div class="my-5">
                            <div class="mb-4">
                                ...
                                <select name="division" id="editUserDivision" class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500">
                                    <option class="text-red-400" value="{{ $editUserInfo->division }}">@if($editUserInfo->area) {{$editUserInfo->area}}, @endif {{$editUserInfo->district}}, {{$editUserInfo->division}}.</option>
                            ...
                            <div class="mb-4">
                                <select name="district" id="editUserDistrict" class="hidden w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500">
                                    ...
                                    <option class="text-green-400" value="{{ $editUserInfo->district }}">{{$editUserInfo->district}}</option>
                                </select>
                            </div>
                            <div class="mb-4">
                                <select name="area" id="editUserArea" class="hidden w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500">
                                    @if($editUserInfo->area)

                                    <option class="text-green-400" value="{{ $editUserInfo->area }}">{{$editUserInfo->area}}</option>
                                    @else
